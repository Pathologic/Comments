<form method="post">
    ...
    <div class="form-group">
        ...
        <textarea class="form-control" id="comment" placeholder="Напишите сообщение" rows="10" name="comment" style="resize: none;">{{ data.comment }}</textarea>
            ...
            <div class="col-xs-6">
                <div class="form-group">
                    <label class="checkbox-inline">
                        <input type="checkbox" name="published" value="1" {{ data.published ? 'checked' : '' }}> Опубликован
                    </label>
                </div>
            </div>
            <div class="col-xs-6">
                <div class="form-group">
                    <label class="checkbox-inline">
                        <input type="checkbox" name="deleted" value="1" {{ data.deleted ? 'checked' : '' }}> Удален
                    </label>
                </div>
            </div>
        </div>
    ...
    <div class="container-fluid">
        <div class="row form-group">
            ...
            <div class="col-xs-6">
                <b>Ресурс:</b> {{ data.thread }} {% if data.context == 'site_content' and data.resource %}(<a href="{{ constant('MODX_MANAGER_URL') ~ 'index.php?a=27&id=' ~ data.thread }}" target="main">{{ data.resource }}</a>){% endif %}
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-4">
                <b>Создан:</b><br>{{ data.createdon }}<br>
                ...
                {% elseif  data.createdby == 0 %}
                    <span class="guest">Гость {{ data.name }}</span>
                {% else %}
                    {{ data.createdby }}
                {% endif %}
            </div>
            {% if data.updatedby != 0 %}
                <div class="col-xs-4">
                    <b>Изменен:</b><br>{% if data.updatedon != '0000-00-00 00:00:00' %}{{ data.updatedon }} <br>{% endif %}
            ...
            {% if data.deletedby != 0 %}
                <div class="col-xs-4">
                    <b>Удален:</b><br>{% if data.deletedon != '0000-00-00 00:00:00' %}{{ data.deletedon }} <br>{% endif %}{% if data['user.username.deletedby'] %}
